...
const HeaderActions = ({ onSummaryPress, onPdfPress, data, nombreUsuario }: any) => (
  <View style={styles.headerRow}>
    ...
    <DownloadDietPdfButton data={data} nombreUsuario={nombreUsuario} style={styles.headerButton} title="PDF" iconSize={width * 0.05} />
  </View>
);
    ...
    <LinearGradient colors={PALETTE.background_gradient} style={styles.base}>
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        ...
        >
          ...
          <Text style={styles.footerText}>Generada el: {new Date(data.creado).toLocaleString()}</Text>
        </ScrollView>
      </KeyboardAvoidingView>
    </LinearGradient>
  );
